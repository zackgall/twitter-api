...
var mongoURL = 'mongodb://fxadmin:Password1@ds133231.mlab.com:33231/replaytweets'
var orderCollected = 0;

//define find method


// Twitter Stream Topics
var trackedTags = ['#AcademyPgh', '#AcademyPgh3']; // topics
        ...
        stream.on('data', function(data) {
            ...
            trackedTags.forEach(function(trackedTags) {
                ...
                if (data.text.match(myRegExp) && data.entities.media != null) {
                    ...
                    collection.insert({
                            'postDT': moment().format('MM-DD-YYYY HH:MM'), // GEO?
                            'userName': data.user.screen_name,
                            'userImage': data.user.profile_image_url,
                            'tweetImage': data.entities.media[0].media_url_https,
                            'tweetText': data.text
                        },
                        function(err, result) {
                            if (err) {
